...
To inspect what the AV says about the question or answer tokens in a base-model
run, use the answer probe demo. It lets the base model generate first, extracts
question and/or answer-token activations at the NLA layer, then prints the AV's
natural-language description for selected token positions:
...
Use `--target question` for the user's prompt, `--target answer` for the
model's generated answer, `--target prompt` for the full formatted prompt
(system / role / newline tokens included), or `--target all` for formatted
prompt plus answer.
Use `--positions all` (default), `--positions 0,3,5-9`, `--start`, and `--n`
to choose which target-token positions to verbalize. Add `--quiet --out ...`
to export the full result without printing long AV explanations to the terminal.
Omit `--ar` for AV-only output without reconstruction scores.

For an interactive VSCode Remote SSH workflow, use the small terminal UI:
...
It exposes prompt text, AR scoring, extraction layer, answer temperature, AV
temperature, max answer tokens, and output path. It keeps the fixed defaults at
`--target all --positions all --quiet`, so full-context AV results are exported
to JSON instead of flooding the terminal. The released Qwen AV/AR checkpoints
are trained for layer 20; setting layer 21 in the UI is an intentional mismatch
probe.
