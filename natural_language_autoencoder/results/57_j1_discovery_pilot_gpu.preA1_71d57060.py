...
def extract_candidate_vectors(*, contexts: list[dict[str, Any]], docs: dict[int, dict[str, Any]],
                               ...
                               batch_size: int = 8, rtol: float = 5e-3,
    ...
    for row in contexts:
        doc_id = int(row["doc_id"])
        ids = token_cache.setdefault(doc_id, _tokenize_full(tok, docs[doc_id]["text"]))
        position = int(row["position"])
        if position >= len(ids):
            errors.append({"kind": "position_out_of_range", "feature": int(row["feature"]),
                           "doc_id": doc_id, "position": position, "token_count": len(ids)})
            continue
        actual_token_id = int(ids[position])
        if actual_token_id != int(row["token_id"]):
            errors.append({"kind": "token_id_mismatch", "feature": int(row["feature"]),
        ...
        if not piece or local < 0 or local >= len(piece) or len(piece) > CHUNK_LEN:
            errors.append({"kind": "invalid_chunk_position", "feature": int(row["feature"]),
                           "doc_id": doc_id, "position": position,
    ...
    for row in contexts:
        ...
        if not np.isclose(actual, expected, rtol=rtol, atol=atol):
            errors.append({"kind": "activation_mismatch", "context_index": index,
                           ...
                           "actual": actual, "abs_error": abs(actual - expected),
                           "rtol": rtol, "atol": atol})
    if errors:
        raise ExtractionVerificationError(errors)

    feature_ids = sorted({int(row["feature"]) for row in contexts})
    ...
    for fi, feature in enumerate(feature_ids):
        ...
        discovery_rows = [row for row in contexts
...
def _freeze_payload(*, selected: list[dict[str, Any]], contexts: list[dict[str, Any]],
    ...
    for feature_row in selected:
        feature = int(feature_row["feature"])
        feature_contexts = [row for row in contexts if int(row["feature"]) == feature]
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--activation-rtol", type=float, default=5e-3)
