...
log=$results/n5_heldout_reconstruct_v2.log
status=$results/n5_heldout_reconstruct_v2.exit
...
if /root/miniconda3/bin/python -B 44_n5_reconstruct.py \
    ...
    --checkpoint "$results/n5_heldout_av_checkpoint_v2.jsonl" \
    --explanations-out "$results/n5_heldout_explanations_v2.json" \
    --variants-out "$results/n5_heldout_variants_v2.json" \
    --out "$results/n5_heldout_recon_v2.json" \
    --vecs-out "$results/n5_heldout_recon_vectors_v2.npz" \
    >"$log" 2>&1; then
    printf '0\n' >"$status"
else
    exit_code=$?
